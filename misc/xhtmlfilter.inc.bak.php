...
{
	function xhtmlfilter_elementpattern($pattern)
	{
		...
		{
			if (is_array($val)) $pattern['contents'][$key] = new xhtmlfilter_elementpattern($val);
		}
		...
		{               
			...
			do
			{
				...
				$match = is_object($el)
					...
					: ($el == $element ? PATTERNMATCH_OK : PATTERNMATCH_BADMATCH);
				
				// if there's no match, and we're not already part way through a sequence, return NOMATCH
				if ($match != PATTERNMATCH_OK and $match != PATTERNMATCH_NOMATCH and ($this->pattern['quant'] == '*' or $this->pattern['quant'] == '?') and $this->pattern == $this->original)
				{
...
class xhtmlfilter
{
	function xhtmlfilter($value = NULL, $dtd = 'xhtml1-strict')
	{
		$this->value = $value;
		if (strpos($dtd, '/') !== false) $dtd = 'xhtml1-strict';
		...
		if (preg_match_all('/&([a-zA-Z\#][a-zA-Z0-9]*)\;/', $value, $matches))
		{
			foreach ($matches[1] as $key => $val)
			{
				if ($val[0] == '#')
				{
					...
					{
						$num = $numparts[1]{0} == 'x' ? hexdec(substr($numparts[1]), 1) : (int)$numparts[1];
						if ($num < 32 or ($num >= 127 and $num <= 159) or $num >= 0xfffe) $value = str_replace($matches[0][$key], '', $value);
					}
				}
				else
				{
	...
	function &gettag($advance = true)
	{
		...
		$i = $this->i;
		$char = $this->value[$i];
		if ($char != '<' or (strpos('/!apdtbsiefAPDTBSIEFcghjklmnoqruvwxyzCGHJKLMNOQRUVWXYZ', $this->value[$i+1]) === false)) // not a valid tag
		// string literal
		{
			...
			$i = $end;
			$tag = &new xhtml_element('#PCDATA', $strval);
			if ($advance)	$this->i = $i;
		...
		// comment or CDATA
		{
			...
			if ($end === false) $end = strlen($this->value);
			$i = $end + 1;
			$tag = &new xhtml_element($tagtype, $data);
			if ($advance)	$this->i = $i;
				else $this->currenttag = $tag;
			return $tag;
		}
		...
		if (!$result)
		{
			$tag = &new xhtml_element('#PCDATA', '&lt;');
			if ($advance)	$this->i = $i;
				else $this->currenttag = $tag;
			return $tag;
		}
		$element = new xhtml_element(strtolower($matches[2]));
		$i += strlen($matches[2]);
	...
	{
		if (in_array($parent, array('p', 'li', 'option', 'td', 'a')) and $parent == $child) return true;
		...
		if (in_array($parent, array('h1', 'h2', 'h3', 'h4', 'h5', 'h6')) and in_array($child, array('h1', 'h2', 'h3', 'h4', 'h5', 'h6'))) return true;
		return false;
	}
	...
	function &getelement()
	{
		...
		if (count($tagstack) and in_array(end($tagstack), array('p', 'li', 'option', 'dd', 'dt', 'head', 'body', 'td', 'a', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6')))
		{                                   
		...
		{
			$element = &$this->gettag(false);
			//if ($element->close and !$element->open and $element->element != end($tagstack) and ($element->element == current(array_slice($tagstack, -2, 1)))) return NULL;
			if ($element->close and !$element->open and $element->element != end($tagstack) and in_array($element->element, $tagstack)) return NULL;
		}
		// consume the next tag
		$element = &$this->gettag();
		if ($element->element == '_invalid' or ($element->close and !$element->open and $element->element == end($tagstack))) return NULL;
		if ($element->close and !$element->open) return new xhtml_element('_invalid');
		...
		if (count($tagstack) > 256) exit('Error: too much recursion');
	...
	{
		if (isset($element->attributes['for']) and empty($this->elementids[$element->attributes['for']]))
	...
	function dofilterxhtml(&$element)
	{
	  ...
	  $patt = (isset($this->elements[$element->element])) ? $this->elements[$element->element] : 'EMPTY';
		$pattern = new xhtmlfilter_elementpattern($patt);
		...
		while (count($filterbuffer) and ($tries-- > 0))
	  {
	  	$child = array_shift($filterbuffer);
			...
			if ($this->keepwhitespace and $child->element == '#PCDATA' and strlen(trim($child->data)) == 0)
			// just whitespace
  		{
	  	...
	  	else
	  	{
            ...
            break;
  			...
  			elseif ($child->element == 'li' and $pattern->trymatch('ul', false) > 0) $hint = 'ul';
  			elseif ($child->element == 'td' and $pattern->trymatch('tr', false) > 0) $hint = 'tr';
...
set_time_limit(1);
$xhtmlfilter = new xhtmlfilter('<html><head><title>some title</title><body><p>Some paragraph</p></body></html>');
